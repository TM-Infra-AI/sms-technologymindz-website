...
echo $number=$_POST['number1'];

echo $msg=$_POST['msg'];
echo $twilio_num=$_POST['twilio_num'];
...
$sqli = mysqli_query($con,"insert into table_data(sender,body,date_time,sending_status) values('$number','$msg',now(),'$status')");
...
// echo "insert into table_data(sender,body,date_time,sending_status) values('$number','$msg',now(),'$status')";
    ...
    $params = array(
        'src' => $twilio_num, // Sender's phone number with country code
...
$sqli12 = mysqli_query($con,"insert into table_data(sender,body,date_time,sending_status) values('$number','$msg',now(),'$status')");
...
// // echo "select * from  tapp_sent_msg order by date_time desc limit 80";
...
//         'url' => 'https://sms.technologymindz.com/status_report_chat.php', // The URL to which with the status of the message is sent
//         'method' => 'POST' // The method used to call the url
//     );
//     // Send message
//     $response = $p->send_message($params);
