...
$year=$_POST['year'];
$query=mysqli_query($con,"SELECT * FROM tapp_msg_receive WHERE MONTH( date_time ) =$month AND YEAR( date_time ) =$year ORDER BY date_time DESC");
    }
 ...
 $query=mysqli_query($con,"select * from tapp_msg_receive where sender='".$msg_num."' OR body like '%".$msg_num."%'  order by date_time desc");
    ...
    {
    $query=mysqli_query($con,"SELECT *  FROM tapp_msg_receive ORDER BY date_time DESC");  
    }
...
?>
                  <tr id="tr" class="tr<?php echo $row['id'];?>" style="font-size:14px">
                    ...
                    <!-- <td><a href="chat.php?number=<?php echo $data['sender'];?>" ><i class="fa fa-reply" aria-hidden="true"></i></a></td> -->


                 <td nowrap="nowrap"><a href="chat.php?number=<?php echo $data['sender'];?>" class="btn-success btn-sm" >Reply
               </a></td>




                    <td nowrap="nowrap"><a href="delete_inbox_msg.php?id=<?php echo $data['id'];?>" onclick="return confirm('Are you sure you want to delete?')" class="btn-danger btn-sm" >Delete
               </a></td>
                  </tr>
