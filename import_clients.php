...
if(mysqli_num_rows($check_dup)<1 && $sms_number != '' && $email != '')
{
	$counter = $counter + 1;
// 	echo "insert";
// die;
$sql=mysqli_query($con,"insert into tapp_tbl_clients (sender,first_name,email,address,date_time) values('".$sms_number."','".$first_name."','".$email."','".$candidate_location."',now())")or die(mysqli_error($con));
}
if($sql)
{
 $_SESSION['flash_message'] = 'Congrats! '.$counter.' Clients imported successfully.';
}
else{
